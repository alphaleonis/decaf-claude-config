...
**Query tools:**
- `mcp__memory-service__memory_search` - Semantic search by topic/keyword (preferred — finds conceptually similar results even with different wording)
- `mcp__memory-service__memory_list` - Browse patterns filtered by tags
...
→ Search: mcp__memory-service__memory_search({query: "polly retry"})
...
- `mcp__memory-service__memory_search` - Semantic search for patterns
- `mcp__memory-service__memory_store` - Create new pattern
- `mcp__memory-service__memory_update` - Update existing pattern
- `mcp__memory-service__memory_graph` - Explore connections between patterns
