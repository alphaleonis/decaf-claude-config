...
1. **Search the knowledge graph** using `mcp__plugin_decaf-memory-doobidoo_memory__memory_search` with the provided query (mode: `semantic` for best results)
...
- `mcp__plugin_decaf-memory-doobidoo_memory__memory_search` - Semantic search for patterns by keyword (default mode)
- `mcp__plugin_decaf-memory-doobidoo_memory__memory_list` - Browse stored patterns with tag filtering
