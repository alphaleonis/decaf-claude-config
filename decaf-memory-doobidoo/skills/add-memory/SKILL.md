...
5. **Check for existing** - Search for similar patterns first using `mcp__memory-service__memory_search`
   - If exists: Use `mcp__memory-service__memory_update` to extend
   - If new: Use `mcp__memory-service__memory_store` to create
...
- `mcp__memory-service__memory_search` - Check for existing patterns (semantic search)
- `mcp__memory-service__memory_store` - Create new pattern
- `mcp__memory-service__memory_update` - Update existing pattern metadata/tags
